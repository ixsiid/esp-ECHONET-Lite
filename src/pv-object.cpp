...
PV::PV(uint8_t instance) : ELObject(instance, PV::class_u16) {
	...
	// 状態変更通知アナウンスマップ
	props[0x9d] = new uint8_t[0x04]{0x03, 0x02, 0x80, 0xb1};
	// Setプロパティマップ
	props[0x9e] = new uint8_t[0x05]{0x04, 0x03, 0xa0, 0xa1, 0xc1};
	// Getプロパティマップ
	...
	accumulation_watt_tick = 0;
};

void PV::update() {
	// 積算値を更新する 0xe1: Wh 4byte
	portTickType t = xTaskGetTickCount();
	accumulation_watt_tick += (props[0xe0][1] << 8 | props[0xe0][0]) * (t - timer);

	// 1000な気がするけど10になるのはなぞ
	uint32_t wh = accumulation_watt_tick / portTICK_PERIOD_MS / 3600 / 10;
	props[0xe1][1] = wh >> 24;
	props[0xe1][2] = wh >> 16;
	props[0xe1][3] = wh >> 8;
	props[0xe1][4] = wh >> 0;
